...
{
    CARD_REG16_OFF(GET_CARD_REG_ADDR(A_PAD_TOP_BANK, 0x00), BIT01_T);         //reg_all_pad_in => Close
    CARD_REG16_OFF(GET_CARD_REG_ADDR(A_PMU_BANK, 0x7F), BIT03_T);             //reg_all_pad_in => Close

    //SD0 Power Off
    CARD_REG16_OFF(GET_CARD_REG_ADDR(A_PMU_BANK, 0x7E), BIT00_T);     //CARD_POWER_CTL1 (gpio mode), all pad in
    CARD_REG16_ON(GET_CARD_REG_ADDR(A_PMU_BANK, 0x7D), BIT00_T);      // output:0
...
void Hal_CARD_InitPADPin(PADEmType ePAD, BOOL_T bTwoCard)
{
    ...
    if(ePAD == EV_PAD1) //Pad SD0
    {
        ...
        CARD_REG16_ON(GET_CARD_REG_ADDR(A_PAD_TOP_BANK, 0x07), BIT13_T|BIT12_T|BIT11_T|BIT10_T|BIT09_T|BIT08_T);   //D3, D2, D1, D0, CMD, CLK => pull en
        CARD_REG16_ON(GET_CARD_REG_ADDR(A_PAD_TOP_BANK, 0x08), BIT05_T|BIT04_T|BIT03_T|BIT02_T|BIT01_T);   //D3, D2, D1, D0, CMD => pull up
        //CARD_REG16_ON(GET_CARD_REG_ADDR(A_PAD_TOP_BANK, 0x07), BIT05|BIT04|BIT03|BIT02|BIT01);   //D3, D2, D1, D0, CMD => drv strengh
        CARD_REG16_OFF(GET_CARD_REG_ADDR(A_PAD_TOP_BANK, 0x08), BIT00_T);   //CLK => pull down
        //CARD_REG16_ON(GET_CARD_REG_ADDR(A_PAD_TOP_BANK, 0x07), BIT00);   //CLK => drv strengh
        CARD_REG16_ON(GET_CARD_REG_ADDR(A_PMU_BANK, 0x7F), BIT01_T|BIT00_T);   //CDZ pull up/ pull en
    }
...
void Hal_CARD_SetPADToPortPath(IPEmType eIP, PortEmType ePort, PADEmType ePAD, BOOL_T bTwoCard)
{
    ...
    if(eIP == EV_IP_FCIE1)
    {
        if(ePort == EV_PORT_SDIO1)
        {
            if(ePAD == EV_PAD1) //Pad SD0
            {
                CARD_REG16_ON(GET_CARD_REG_ADDR(A_PAD_TOP_BANK, 0x01), BIT10_T);   //reg_sdio_mode = 1
                CARD_REG16_OFF(GET_CARD_REG_ADDR(A_PAD_TOP_BANK, 0x01), BIT11_T);
                CARD_REG16_ON(GET_CARD_REG_ADDR(A_PMU_BANK, 0x7C), BIT07_T);       //sd_cdz
            }
        }
    }
    else if(eIP == EV_IP_FCIE2)
...
void Hal_CARD_PullPADPin(PADEmType ePAD, PinEmType ePin, PinPullEmType ePinPull, BOOL_T bTwoCard)
{
    ...
    if(ePAD == EV_PAD1) //PAD_SD0
    {
        if(ePinPull ==EV_PULLDOWN)
        {
            ...
            {
                CARD_REG16_OFF(GET_CARD_REG_ADDR(A_PAD_TOP_BANK, 0x08), BIT05_T|BIT04_T|BIT03_T|BIT02_T|BIT01_T);   //D3, D2, D1, D0, CMD => pull down
            }
        }
...
void Hal_CARD_SetClock(IPEmType eIP, U32_T u32ClkFromIPSet)
{
    ...
    if(eIP == EV_IP_FCIE1)
    {
        CARD_REG16_OFF(GET_CARD_REG_ADDR(A_CHIPTOP_BANK,0x25),BIT10_T|BIT09_T|BIT08_T|BIT07_T|BIT06_T); //FCIE :300KHz
        CARD_REG16_OFF(GET_CARD_REG_ADDR(A_CHIPTOP_BANK,0x30), BIT10_T|BIT09_T|BIT08_T|BIT07_T|BIT06_T|BIT05_T|BIT04_T|BIT03_T|BIT02_T|BIT01_T|BIT00_T);


        switch(u32ClkFromIPSet)
        {
            case CLK1_F:      //48000KHz
                CARD_REG16_ON(GET_CARD_REG_ADDR(A_CHIPTOP_BANK,0x25),BIT10_T|BIT09_T);
                break;
            case CLK1_E:      //40000KHz
                CARD_REG16_ON(GET_CARD_REG_ADDR(A_CHIPTOP_BANK,0x25),BIT10_T|BIT08_T);
                break;
            case CLK1_D:      //36000KHz
                CARD_REG16_ON(GET_CARD_REG_ADDR(A_CHIPTOP_BANK,0x25),BIT10_T);
                break;
            case CLK1_C:      //32000KHz
                CARD_REG16_ON(GET_CARD_REG_ADDR(A_CHIPTOP_BANK,0x25),BIT09_T|BIT08_T);
                break;
            case CLK1_B:      //27625KHz ==> spread clock
                CARD_REG16_ON(GET_CARD_REG_ADDR(A_CHIPTOP_BANK,0x25),BIT10_T|BIT09_T|BIT08_T);
                CARD_REG16_ON(GET_CARD_REG_ADDR(A_CHIPTOP_BANK,0x30), BIT03_T|BIT00_T);
                break;
            case CLK1_A:      //24000KHz
                CARD_REG16_ON(GET_CARD_REG_ADDR(A_CHIPTOP_BANK,0x25),BIT09_T);
                break;
            case CLK1_9:      //18420KHz ==> spread clock
                CARD_REG16_ON(GET_CARD_REG_ADDR(A_CHIPTOP_BANK,0x25),BIT10_T|BIT09_T|BIT08_T);
                CARD_REG16_ON(GET_CARD_REG_ADDR(A_CHIPTOP_BANK,0x30), BIT03_T|BIT02_T|BIT00_T);
                break;

            case CLK1_8:      //15790KHz ==> spread clock
                CARD_REG16_ON(GET_CARD_REG_ADDR(A_CHIPTOP_BANK,0x25),BIT10_T|BIT09_T|BIT08_T);
                CARD_REG16_ON(GET_CARD_REG_ADDR(A_CHIPTOP_BANK,0x25),BIT10_T|BIT09_T|BIT08_T);
                CARD_REG16_ON(GET_CARD_REG_ADDR(A_CHIPTOP_BANK,0x30), BIT03_T|BIT02_T|BIT01_T|BIT00_T);
                break;

            case CLK1_7:      //12000KHz
                CARD_REG16_ON(GET_CARD_REG_ADDR(A_CHIPTOP_BANK,0x25),BIT08_T);
                break;
...
void Hal_CARD_PowerOn(PADEmType ePAD, U16_T u16DelayMs)
{
    ...
    {
        CARD_REG16_OFF(GET_CARD_REG_ADDR(A_PMU_BANK, 0x7E), BIT00_T);            //CARD_POWER_CTL1 (gpio mode), all pad in
        CARD_REG16_OFF(GET_CARD_REG_ADDR(A_PMU_BANK, 0x7D), BIT00_T);            // output:0
