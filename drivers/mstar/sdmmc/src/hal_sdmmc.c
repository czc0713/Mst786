...
#define V_MIE_INT_EN_INIT   (R_SD_CMD_END_EN | R_SDIO_INT_EN)
...
static volatile BOOL_T  gb_SDIOClk[3] = {0};
...
static volatile U16_T   gu16_WT_NAC[3] = {0};             //Waiting Time for Nac (us)
...
static void _REG_ResetIP(IPEmType eIP)
{
	...
	CARD_REG16_OFF(A_MIE_PATH_CTL_REG(eIP), R_MMA_ENABLE);       //Avoid MMA_Enable lock (Auto Clear Fail)
	CARD_REG16_OFF(A_SD_CTL_REG(eIP), R_SD_CMD_EN|R_SD_DTRX_EN); //Avoid SD_CMD_EN/DTRX_EN lock (Auto Clear Fail)
	CARD_REG16_OFF(A_MMA_PRI_REG_REG(eIP), R_MIU_CLK_EN_HW);     //Avoid MIU_CLK_EN_HW lock (Auto Clear Fail)

	CARD_REG16_ON(A_MMA_PRI_REG_REG(eIP), R_MIU_REQ_RST);        //Avoid MIU Request lock (MIU hold)
...
void Hal_SDMMC_SetSDIOIntDet(IPEmType eIP, BOOL_T bEnable)
{
	...
	if(bEnable)
		gu16_SD_CTL_SDIO_Det[eIP] = R_SDIO_DET_ON/*|R_SDIO_INT_MOD0|R_SDIO_INT_MOD1*/ ;
	else
		gu16_SD_CTL_SDIO_Det[eIP] = 0;

}

void Hal_SDMMC_SetSDIOClk(IPEmType eIP, BOOL_T bEnable)
{
	if(bEnable)
		gb_SDIOClk[eIP] = TRUE;
	else
		gb_SDIOClk[eIP] = FALSE;
...
#if(EN_BYTEMODE)
		CARD_REG(A_SDIO_ADDR1_REG(eIP)) = (U16_T)(u32Addr>>16);
		CARD_REG(A_SDIO_ADDR0_REG(eIP)) = (U16_T)(u32Addr);
		CARD_REG(A_SDIO_CTL_REG(eIP)) = u16BlkSize | R_SDIO_BLK_MOD;
#else
		CARD_REG(A_MIU_DMA1_REG(eIP)) = (U16_T)(((u32Addr&0xFFFFFFF)/D_MIU_WIDTH)>>16);
		CARD_REG(A_MIU_DMA0_REG(eIP)) = (U16_T)((u32Addr)/D_MIU_WIDTH);
		CARD_REG(A_DAM_OFFSET_REG9(eIP)) = u8BitMask[u32Addr&(D_MIU_WIDTH-1)];
...
RspErrEmType Hal_SDMMC_SendCmdAndWaitProcess(IPEmType eIP, TransEmType eTransType, CmdEmType eCmdType, SDMMCRspEmType eRspType, BOOL_T bCloseClk)
{
	...
	CARD_REG16_ON(A_EMMC_BOOTCFG_REG(eIP), V_SD_BOOTCFG_INIT);

	CARD_REG(A_SD_MODE_REG(eIP)) = V_SD_MODE_INIT | u16VAR_SD_MODE | (eTransType>>8) | (gb_SDIOClk[eIP] ? 0 : (u16MIE_PATH_MMA_EN<<11));
	...
	CARD_REG(A_SD_CTL_REG(eIP))  = V_SD_CTL_INIT | gu16_SD_CTL_SDIO_Det[eIP] | (eRspType>>12) | (eCmdType>>4);
	...
	if ( (eTransType== EV_DMA) && _REG_WaitFIFOClkRdy(eIP, eCmdType, WT_FIFO_CLK_RDY) )
		return _SDMMC_ErrProcess(eIP, eCmdType, EV_STS_FIFO_NRDY, bCloseClk, __LINE__);
	...
	if(eCmdType==EV_CMDREAD)
	{
		...
		u32WaitMS = WT_EVENT_RSP + WT_EVENT_READ;
...
{
	CARD_REG16_ON(A_MIE_PATH_CTL_REG(eIP), V_MIE_PATH_INIT);
	CARD_REG16_ON(A_SD_MODE_REG(eIP), _REG_GetPortSetting(eIP));
