 ...
 * Desc:
 * 	   This layer between UBOOT SD Driver layer and IP Hal layer.
...
//-----------------------------------------------------------------------------------------------------------
static const IPEmType ge_IPSlot[3]            = {D_SDMMC1_IP, D_SDMMC2_IP, D_SDMMC3_IP};
static const PortEmType ge_PORTSlot[3]        = {D_SDMMC1_PORT, D_SDMMC2_PORT, D_SDMMC3_PORT};
static const PADEmType  ge_PADSlot[3]         = {D_SDMMC1_PAD, D_SDMMC2_PAD, D_SDMMC3_PAD};
...
static int mmc_send_cmd(struct mmc *mmc, struct mmc_cmd *cmd, struct mmc_data *data)
{
	...
	else  //SD R/W Command and DATA
	{
		...
		{
			eCmdType = EV_CMDREAD;
			pu16Buf = (volatile U16_T *)data->dest;
		}
		else
		{
			eCmdType = EV_CMDWRITE;
			pu16Buf = (volatile U16_T *)data->src;
		}
		...
		Hal_SDMMC_TransCmdSetting(eIP, eTransType, eCmdType, u16BlkCnt, u16BlkSize, Hal_CARD_TransMIUAddr((U32_T)pu16Buf));
