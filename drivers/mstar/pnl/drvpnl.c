...
//  Local Structurs
//-------------------------------------------------------------------------------------------------
static const U8       _cPnlVer[] = PNL_LIB_VERSION;
//static const PNL_DrvInfo _cstPnl_DrvInfo = {GAMMA_TYPE, GAMMA_MAPPING};
static PNL_DrvStatus     _stPnl_DrvStatus = {FALSE, FALSE};
static U16 _u16PnlDbgSwitch;

// Internal Data
//extern U32 gu32PnlRiuBaseAddr ;
//extern U32 gu32PMRiuBaseAddr;
...
U8 g_bDrvPnl_UseTConTable_Mod = FALSE;
//-------------------------------------------------------------------------------------------------
//  Local Variables
//-------------------------------------------------------------------------------------------------
...
#if 0
extern U8 MApi_XC_MLoad_WriteCmds_And_Fire(U32 *pu32Addr, U16 *pu16Data, U16 *pu16Mask, U16 u16CmdCnt);
extern U8 MApi_XC_MLoad_WriteCmd_And_Fire(U32 u32Addr, U16 u16Data, U16 u16Mask);
extern MLOAD_TYPE MApi_XC_MLoad_GetStatus(void);
extern MLG_TYPE MApi_XC_MLG_GetStatus(void);
extern void MApi_XC_MLG_Fire(U8 *pR, U8 *pG, U8 *pB, U16 u16Count, U16 *pMaxGammaValue);
#endif
//-------------------------------------------------------------------------------------------------
//  Debug Functions
//-------------------------------------------------------------------------------------------------
...
static void _MDrv_PNL_Init_XC_VOP(PNL_InitData *pstPanelInitData)
{
    ...
    U16 u16DE_HEND = (pstPanelInitData->u16HStart + pstPanelInitData->u16Width - 1);
    U16 u16DE_HStart = pstPanelInitData->u16HStart;
    U16 u16DE_VEND = (pstPanelInitData->u16VStart + pstPanelInitData->u16Height - 1);
    U16 u16DE_VStart = pstPanelInitData->u16VStart;
...
static void _MDrv_PNL_Init_LPLL(PNL_InitData *pstPanelInitData)
{
    ...
    ldHz = (U64) (((U64)pstPanelInitData->u16HTotal) * pstPanelInitData->u16VTotal * u16DefaultVFreq);
...
static U64 _MDrv_PNL_DClkFactor(PNL_TYPE eLPLL_Type, U32 ldHz)
{
    return (((U64)LVDS_MPLL_CLOCK_MHZ * 524288 * MHal_PNL_Get_LPLL_LoopGain(eLPLL_Type, ldHz)));  //216*524288*8=0x36000000
}
...
/*static void _MDrv_PNL_TCON_GetTabSize(U8 *pTconTab, U16 u16tabtype, U32 *pu32TconTabsize)
{
    U8 u8Colslen = 0;
    U16 u16timeout = 0x3FFF;
    U16 u16Addr;
    if (pTconTab == NULL)
    {
...
//  Global Functions
//-------------------------------------------------------------------------------------------------
const U8 * MDrv_PNL_GetLibVer(void)              ///< Get version
{
    return _cPnlVer;
}

U8 MDrv_PNL_GetStatus(PNL_DrvStatus *pDrvStatus)    ///< Get panel current status
{
...
U8 MDrv_PNL_SetDbgLevel(U16 u16DbgSwitch)       ///< Set debug level
{
...
U8 MDrv_PNL_Control_Out_Swing(U16 u16Swing_Level)
{
    return (U8)MHal_PNL_MOD_Control_Out_Swing(u16Swing_Level);
}
...
// Output Dclk
U32 MDrv_PNL_CalculateLPLLSETbyDClk(U32 ldHz)
{
    ...
    switch(_stPnlInitData.eLPLL_Type)
    {
        ...
        default:
            ...
            u32div=(U32)(MHal_PNL_Get_Loop_DIV(_stPnlInitData.eLPLL_Mode, _stPnlInitData.eLPLL_Type, ldHz));
...
{
    U32 u32NonPMBankSize, u32PMBankSize;
#if 0
    // get MMIO base
...
#endif
    gu32PMRiuBaseAddr=0x1f000000;
    gu32PnlRiuBaseAddr=0x1f200000;
...
PNL_Result MDrv_PNL_SetPanelType(PNL_InitData *pstPanelInitData)
{
    ...
    {
    #define REG_SC_BK00_06_L (0x130000+(0x06*2))
    #define REG_SC_BK10_23_L (0x131000+(0x23*2))
    //#define REG_SC_BK10_19_L (0x131000+(0x19*2))

	U32 u32ModuleBaseAddr =0x1F000000;

	OUTREG16((u32ModuleBaseAddr + 2*REG_SC_BK00_06_L), 0xF008);
	OUTREG16((u32ModuleBaseAddr + 2*REG_SC_BK10_23_L), 0x0080);
	//OUTREG16((u32ModuleBaseAddr + 2*REG_SC_BK10_19_L), 0x0000);
    }
    return E_PNL_OK;
}
...
static void _MDrv_VOP_SetGammaMappingMode(U8 u8Mapping)
{
    MHal_VOP_SetGammaMappingMode(u8Mapping);
}
#if 0
/// set gamma table
PNL_Result MDrv_PNL_SetGammaTbl( DRVPNL_GAMMA_TYPE eGammaType, U8* pu8GammaTab[3],
                                 DRVPNL_GAMMA_MAPPEING_MODE GammaMapMode )
{
    U8 u8TgtChannel = 0;
    U8* pcTab1;
    U8 u8BackupMainSubEnableFlag = 0;
    ...
    {
        U8 u8Channel;
        U16 pMaxGammaValue[3] ;
        U16 u16NumOfLevel = ((GammaMapMode == E_DRVPNL_GAMMA_8BIT_MAPPING) ? 256 : 1024);
...
PNL_Result MDrv_PNL_SetGammaValue(U8 u8Channel, U16 u16Offset, U16 u16GammaValue)
{
    U16 u16MaxGammaValue;
    U8 u8BackupMainSubEnableFlag = 0;
    U16 u16MaxOffset = 0;
    U8 bUsingBurstWrite = FALSE;
...
#endif
PNL_Result MDrv_PNL_En(U8 bPanelOn, U8 bCalEn)
{
...
PNL_Result MDrv_PNL_SetSSC(U16 u16Fmodulation, U16 u16Rdeviation, U8 bEnable)
{
    U16 u16Span;
    U16 u16Step;
    U32 u32PLL_SET;/// = MDrv_Read3Byte(L_BK_LPLL(0x0F));
    ...
    if(u16Fmodulation < 200 || u16Fmodulation > 400)
        u16Fmodulation = 300;
    u16Span =(U16)( ( (((U32)LVDS_MPLL_CLOCK_MHZ*LVDS_SPAN_FACTOR ) / (u16Fmodulation) ) * 10000) / ((U32)u32PLL_SET) ) ;

    // Set STEP
    if(u16Rdeviation > 300)
        u16Rdeviation = 300;
    u16Step = (U16)(((U32)u32PLL_SET*u16Rdeviation) / ((U32)u16Span*10000));
...
void MDrv_PNL_SetOutputPattern(U8 bEnable, U16 u16Red , U16 u16Green, U16 u16Blue)
{
...
#if 0 // mode to APP
static void _MDrv_PNL_TCON_DumpPSRegTab(U8 u8TconTab[TCON_TAB_MAX_SIZE], U8 u8Tcontype, U8 *pu8delay)
{
    U32 u32tabIdx = 0;
    U16 u16timeout = 0x3FFF;
    U32 u32Addr;
...
void MDrv_PNL_TCON_Count_Reset( U8 bEnable )
{
...
U8 MDrv_PNL_GetDataFromRegister(PNL_InitData *PNL_Data)
{
...
void MDrv_PNL_SetSSC_En(U8 bEnable)
{
    //printf("bEnable = %d\n", bEnable);
    MHal_PNL_Switch_LPLL_SubBank(0x00);
    W2BYTEMSK((L_BK_LPLL(0x0D)), (bEnable << 11), BIT(11)); // Enable ssc
...
void MDrv_PNL_SetSSC_Fmodulation(U16 u16Fmodulation)
{
    ...
    u16Span =(U16)(( ( (((U32)LVDS_MPLL_CLOCK_MHZ*LVDS_SPAN_FACTOR ) / (u16Fmodulation) ) * 10000) + ((U32)u32PLL_SET/2)) / ((U32)u32PLL_SET))  ;
    //printf("u16Span = 0x%x\n", u16Span);
    W2BYTE(L_BK_LPLL(0x18), u16Span & 0x3FFF);// LPLL_SPAN
...
void MDrv_PNL_SetSSC_Rdeviation(U16 u16Rdeviation)
{
#if 1
    U16 u16Span;
    U16 u16Step;
    ...
    u16Step = (U16)((((U32)u32PLL_SET*u16Rdeviation) + ((U32)u16Span*5000)) / ((U32)u16Span*10000));
    //printf("u16Step = 0x%x\n", u16Step);
    W2BYTE(L_BK_LPLL(0x17), u16Step & 0x03FF);// LPLL_STEP
#endif

}
void MDrv_Mod_Calibration_Init(U8 U8MOD_CALI_TARGET, S8 S8MOD_CALI_OFFSET)
{
    MHal_MOD_Calibration_Init( U8MOD_CALI_TARGET, S8MOD_CALI_OFFSET);
}

void MDrv_BD_LVDS_Output_Type(U16 Type)
{
    MHal_BD_LVDS_Output_Type(Type);
}

U8 MDrv_PNL_SkipTimingChange_GetCaps(void)
{
...
void MDrv_PNL_OverDriver_Init(U32 u32OD_MSB_Addr, U32 u32OD_LSB_Addr, U8 u8ODTbl[1056])
{
    U32 u32OD_MSB_limit, u32OD_LSB_limit;
...
void MDrv_PNL_OverDriver_Enable(U8 bEnable)
{
    if (SUPPORT_OVERDRIVE)
    {
...
U8 MDrv_PNL_Is_SupportFRC(void)
{
    return SUPPORT_FRC;
}
...
int MDrv_PNL_Get_Width()
{
return _stPnlInitData.u16Width;
}
int MDrv_PNL_Get_Height()
{
return _stPnlInitData.u16Height;
}
