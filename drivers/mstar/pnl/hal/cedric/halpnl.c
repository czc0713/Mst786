...
//  Local Variables
//-------------------------------------------------------------------------------------------------
static U16 _u16PnlDbgSwitch = 0;

// Output swing = 150mV + Code * 10mV.
// Designer recommand value. 0x15 = 21 = 250mv
static U16  _u16PnlDefault_SwingLevel = 250;
static PNL_OUTPUT_MODE   _eDrvPnlInitOptions = E_PNL_OUTPUT_CLK_DATA;
U8 _u8MOD_CALI_TARGET = 0x01;     ///< MOD_REG(0x7D),[10:9]00: 250mV ,01: 350mV ,10:300mV ,11:200mV
S8 _usMOD_CALI_OFFSET = 0;        ///< MOD_REG(0x7D),[5:0]+ _usMOD_CALI_OFFSET
U8 _u8MOD_CALI_VALUE  = 0x15;     /// Final value

U8 _u8MOD_LVDS_Pair_Shift = 0;       ///< 0:default setting, LVDS pair Shift
U8 _u8MOD_LVDS_Pair_Type = 0;        ///< 0:default setting, LVDS data differential pair
//-------------------------------------------------------------------------------------------------
//  Debug Functions
//-------------------------------------------------------------------------------------------------
...
U8 MHal_PNL_SetDbgLevel(U16 u16DbgSwitch)       ///< Set debug level
{
...
*
*/
U8 MHal_MOD_PowerOn(U8 bEn, U8 u8LPLL_Type,U8 DualModeType, U16 u16OutputCFG0_7, U16 u16OutputCFG8_15, U16 u16OutputCFG16_21)
{
    if( bEn )
    {
...
*
*/
void MHal_MOD_PVDD_Power_Setting(U8 bIs2p5)
{
...
void MHal_Shift_LVDS_Pair(U8 Type)
{
    if(Type == 1)
    {
...
void MHal_Output_LVDS_Pair_Setting(U8 Type, U16 u16OutputCFG0_7, U16 u16OutputCFG8_15, U16 u16OutputCFG16_21)
{
    // Disable OP
...
void MHal_VOP_SetGammaMappingMode(U8 u8Mapping)
{
...
U8 Hal_VOP_Is_GammaMappingMode_enable(void)
{
    return SC_R2BYTEMSK(REG_SC_BK10_74_L, BIT(15));
}

// After A5, 8 bit mode only support burst write!!!
U8 Hal_VOP_Is_GammaSupportSignalWrite(DRVPNL_GAMMA_MAPPEING_MODE u8Mapping)
{
...
#if 0
void hal_PNL_WriteGamma12Bit(U8 u8Channel, U8 bBurstWrite, U16 u16Addr, U16 u16GammaValue)
{
...
void hal_PNL_SetMaxGammaValue( U8 u8Channel, U16 u16MaxGammaValue)
{
...
static void _Hal_PNL_WriteGamma12Bit(U8 u8Channel, U16 u16Addr, U16 u16GammaValue)
{
    U16 u16Delay = 0xFFFF;
...
static void _Hal_PNL_WriteGamma12Bit_SC1(U8 u8Channel, U16 u16Addr, U16 u16GammaValue)
{
    U16 u16Delay = 0xFFFF;
...
static U16 _Hal_PNL_GetGamma12Data(U8 *pu8GammaTable, U16 u16Index)
{
    U16 u16Data, u16DataOffset;
    ...
    if(u16Index & 1)
    {
        u16Data = (((U16)pu8GammaTable[u16DataOffset+2]) << 4) + (((U16)pu8GammaTable[u16DataOffset]) >> 4);
    }
    else
    {
        u16Data = (((U16)pu8GammaTable[u16DataOffset+1]) << 4) + (((U16)pu8GammaTable[u16DataOffset]) & 0x0F);
    }
...
void Hal_PNL_Set12BitGammaPerChannel( U8 u8Channel, U8 * u8Tab, DRVPNL_GAMMA_MAPPEING_MODE GammaMapMode )
{
    U16 u16Addr            = 0;
    U16 u16GammaValue      = 0;
    U16 u16MaxGammaValue   = 0;
    U16 u16NumOfLevel = GammaMapMode == E_DRVPNL_GAMMA_8BIT_MAPPING ? 128 : 128;
    U16 u16InputData1, u16InputData2, u16OutRef;
    U8  u8OutDelta, u8OutData1, u8OutData2;
    ...
    for(u16Addr=0; u16Addr < u16NumOfLevel; u16Addr++)
    {
        ...
        u8OutData1 = (U8)((u16OutRef & 0x07) << 5) + u8OutDelta;
        u8OutData2 = (U8)(u16OutRef >> 3);

        u16GammaValue =  (((U16)u8OutData2) <<  8) | ((U16)u8OutData1);
...
void MHal_PNL_Init_LPLL(PNL_TYPE eLPLL_Type,PNL_MODE eLPLL_Mode,U64 ldHz)
{
    if((eLPLL_Type == E_PNL_TYPE_LVDS)||(eLPLL_Type == E_PNL_TYPE_HS_LVDS)||(eLPLL_Type == E_PNL_TYPE_HF_LVDS))
    {
        ...
        if(eLPLL_Type == E_PNL_TYPE_LVDS)
        {
			...
			{
				printf(" [LPLL] !!!!! C3: only support *single* LVDS !!!!! \r\n");
			}
        }
...
U8 MHal_PNL_Get_Loop_DIV( U8 u8LPLL_Mode, U8 eLPLL_Type, U32 ldHz)
{
    U8   u8loop_div = 0;
...
U16 MHal_PNL_Get_LPLL_LoopGain(U8 eLPLL_Type, U32 ldHz)
{
    U16     u16loop_gain = 0;
...
U8 Hal_PNL_SkipTimingChange_GetCaps(void)
{
...
// Turn OD function
////////////////////////////////////////////////////////////////////////
void MHal_PNL_OverDriver_Init(U32 u32OD_MSB_Addr, U32 u32OD_MSB_limit, U32 u32OD_LSB_Addr, U32 u32OD_LSB_limit)
{
    SC_W2BYTE(REG_SC_BK16_15_L, (U16)(u32OD_MSB_Addr & 0xFFFF)); // OD MSB request base address
    SC_W2BYTEMSK(REG_SC_BK16_16_L, (U16)((u32OD_MSB_Addr >> 16) & 0x00FF), 0x00FF); // OD MSB request base address
    SC_W2BYTEMSK(REG_SC_BK16_60_L, (U16)((u32OD_MSB_Addr >> 24) & 0x0003), 0x0003); // OD MSB request base address
    SC_W2BYTE(REG_SC_BK16_17_L, (U16)(u32OD_MSB_limit & 0xFFFF)); // OD MSB request address limit
    SC_W2BYTEMSK(REG_SC_BK16_18_L, (U16)((u32OD_MSB_limit >> 16) & 0x00FF), 0x00FF); // OD MSB request address limit
    SC_W2BYTEMSK(REG_SC_BK16_60_L, (U16)((u32OD_MSB_limit >> 24) & 0x0003)<<2, 0x000C); // OD MSB request address limit
    SC_W2BYTE(REG_SC_BK16_39_L, (U16)(u32OD_LSB_limit & 0xFFFF)); // OD frame buffer write address limit
    SC_W2BYTEMSK(REG_SC_BK16_3A_L, (U16)((u32OD_LSB_limit >> 16) & 0x00FF), 0x00FF); // OD frame buffer write address limit
    SC_W2BYTE(REG_SC_BK16_3B_L, (U16)(u32OD_LSB_limit & 0xFFFF)); // OD frame buffer read address limit
    SC_W2BYTEMSK(REG_SC_BK16_3C_L, (U16)((u32OD_LSB_limit >> 16) & 0x00FF), 0x00FF); // OD frame buffer read address limit
    SC_W2BYTE(REG_SC_BK16_4F_L, (U16)(u32OD_LSB_Addr & 0xFFFF)); // OD LSB request base address
    SC_W2BYTEMSK(REG_SC_BK16_50_L, (U16)((u32OD_LSB_Addr >> 16) & 0x00FF), 0x00FF); // OD LSB request base address
    SC_W2BYTEMSK(REG_SC_BK16_50_L, (U16)((u32OD_LSB_limit & 0x00FF) << 8), 0xFF00); // OD LSB request limit address
    SC_W2BYTE(REG_SC_BK16_51_L, (U16)((u32OD_LSB_limit >> 8) & 0xFFFF)); // OD LSB request limit address
...
void MHal_PNL_OverDriver_Enable(U8 bEnable)
{
    // OD mode
...
#endif
U16 _MHal_PNL_MOD_Swing_Refactor_AfterCAL(U16 u16Swing_Level)
{
    U8 u8ibcal = 0x00;
    U16 u16AfterCal_value = 0;
    U16 u16Cus_value = 0;
...
U8 MHal_PNL_MOD_Control_Out_Swing(U16 u16Swing_Level)
{
    U8 bStatus = FALSE;
...
////////////////////////////////////////////////////////////////////////
U8 MHal_PNL_MOD_Control_Out_PE_Current (U16 u16Current_Level)
{
    U8 bStatus = FALSE;
    U16 u16ValidCurrent = u16Current_Level & 0x07;
...
////////////////////////////////////////////////////////////////////////
U8 MHal_PNL_MOD_Control_Out_TTL_Resistor_OP (U8 bEnble)
{
    U8 bStatus = FALSE;
    if(bEnble)
    {
...
void MHal_PNL_SetOutputType(PNL_OUTPUT_MODE eOutputMode, PNL_TYPE eLPLL_Type)
{
        ...
        switch(eOutputMode)
        {
            ...
            default:
				...
				if ( DEVINFO_PANEL_N070ICG_LD1 ==ms_devinfo_panel_type() )
				{
					...
					MOD_W2BYTEMSK(REG_MOD_BK00_75_L, 0x000F, 0xFFFF);
				}
    ...
    {            
	    W2BYTE(REG_GPIO1_16_L, 0x002C);//[5]ttl_bg_mode [3]tck_mode [2]ttlhv_mode  
	    W2BYTE(REG_GPIO0_55_L, 0x0005);//[2:0]Tcon_gpio_4. REset, [10:8]tcon_gpio_5 L/R control		
	    W2BYTE(REG_GPIO0_56_L, 0x0505);	    
    }
	...
	else if(DEVINFO_BD_MST786_CUSTOM40== ms_devinfo_board_type())
    {            
		W2BYTE(REG_GPIO1_16_L, 0x002C);//[5]ttl_bg_mode [3]tck_mode [2]ttlhv_mode     
		W2BYTE(REG_GPIO0_55_L, 0x0101);//[2:0]Tcon_gpio_4. REset, [10:8]tcon_gpio_5 L/R control	
        MHal_GPIO_Pad_Oen(107);
        MHal_GPIO_Set_High(107); 		
	}	
	else	    
    {
		W2BYTE(REG_GPIO1_16_L, 0x002C);//[5]ttl_bg_mode [3]tck_mode [2]ttlhv_mode
...
void Mhal_PNL_Flock_LPLLSet(U32 ldHz)
{
    UNUSED(ldHz);
}


void MHal_PNL_MISC_Control(U32 u32PNL_MISC)
{
...
void MHal_PNL_DumpMODReg(U32 u32Addr, U16 u16Value, U8 bHiByte, U16 u16Mask)
{
    if (bHiByte)
    {
...
void MHal_MOD_Calibration_Init(U8 U8MOD_CALI_TARGET, S8 S8MOD_CALI_OFFSET)
{
    _u8MOD_CALI_TARGET = U8MOD_CALI_TARGET;
    _usMOD_CALI_OFFSET = S8MOD_CALI_OFFSET;
}

void MHal_BD_LVDS_Output_Type(U16 Type)
{
    if(Type == LVDS_DUAL_OUTPUT_SPECIAL )
    {
    ...
    {
        _u8MOD_LVDS_Pair_Type  = (U8)Type;
    }
}

U8 msModCalDDAOUT(void)
{
   // W2BYTEMSK(BK_MOD(0x7D), ENABLE, 8:8);
   // MsOS_DelayTask(10);  //10ms
    return (U8)((MOD_R2BYTEMSK(REG_MOD_BK00_7D_L, BIT(8))) >> 8);
}

U8 msModCurrentCalibration(void)
{
#if 1
#define abs(x) ( (x)<0 ? -(x) : (x))
    ...
    delay_start_time= 0;//MsOS_GetSystemTime();
#endif
    ...
    U8 u8cur_ibcal=0;
    U8 u8cnt;
    U8 bdecflag=0,bincflag=0;
    U8 u8steady=0;
    U16 u16reg_3280,u16reg_3282,u16reg_328a;
    U16 u16reg_32da,u16reg_32dc,u16reg_32de;
    U8 u8preibcal=0;
...
// =========
    ...
    {
        u8cur_ibcal += (U8)abs(_usMOD_CALI_OFFSET);
    ...
    MOD_W2BYTEMSK(REG_MOD_BK00_29_L,((U16)(u8cur_ibcal&0x3F))<<4, 0x3F0);   // ch0
    MOD_W2BYTEMSK(REG_MOD_BK00_29_L,((U16)(u8cur_ibcal&0x3F))<<10, 0xFC00); // ch1

    MOD_W2BYTEMSK(REG_MOD_BK00_2A_L,((U16)(u8cur_ibcal&0x3F)), 0x3F);       // ch2
    MOD_W2BYTEMSK(REG_MOD_BK00_2A_L,((U16)(u8cur_ibcal&0x3F))<<6, 0xFC0);   // ch3
    MOD_W2BYTEMSK(REG_MOD_BK00_2A_L,((U16)(u8cur_ibcal&0x3F))<<12, 0xF000); // ch4
    MOD_W2BYTEMSK(REG_MOD_BK00_2B_L,((U16)(u8cur_ibcal&0x3F))>>4, 0x03);
    MOD_W2BYTEMSK(REG_MOD_BK00_2B_L,((U16)(u8cur_ibcal&0x3F))<<2, 0xFC);    // ch5
    MOD_W2BYTEMSK(REG_MOD_BK00_2B_L,((U16)(u8cur_ibcal&0x3F))<<8, 0x3F00);  // ch6
    MOD_W2BYTEMSK(REG_MOD_BK00_2B_L,((U16)(u8cur_ibcal&0x3F))<<14, 0xC000);   // ch7
    MOD_W2BYTEMSK(REG_MOD_BK00_2C_L,((U16)(u8cur_ibcal&0x3F))>>2, 0x0F);
    MOD_W2BYTEMSK(REG_MOD_BK00_2C_L,((U16)(u8cur_ibcal&0x3F))<<4, 0x3F0);    // ch8
    MOD_W2BYTEMSK(REG_MOD_BK00_2C_L,((U16)(u8cur_ibcal&0x3F))<<10, 0xFC00);    // ch9

    MOD_W2BYTEMSK(REG_MOD_BK00_2D_L,((U16)(u8cur_ibcal&0x3F)), 0x3F);       // ch10
    MOD_W2BYTEMSK(REG_MOD_BK00_2D_L,((U16)(u8cur_ibcal&0x3F))<<6, 0xFC0);   // ch11
    MOD_W2BYTEMSK(REG_MOD_BK00_2D_L,((U16)(u8cur_ibcal&0x3F))<<12, 0xF000); // ch12
    MOD_W2BYTEMSK(REG_MOD_BK00_2E_L,((U16)(u8cur_ibcal&0x3F))>>4, 0x03);
    MOD_W2BYTEMSK(REG_MOD_BK00_2E_L,((U16)(u8cur_ibcal&0x3F))<<2, 0xFC);    // ch13
    MOD_W2BYTEMSK(REG_MOD_BK00_2E_L,((U16)(u8cur_ibcal&0x3F))<<8, 0x3F00);  // ch14
    MOD_W2BYTEMSK(REG_MOD_BK00_2E_L,((U16)(u8cur_ibcal&0x3F))<<14, 0xC000);   // ch15
    MOD_W2BYTEMSK(REG_MOD_BK00_2F_L,((U16)(u8cur_ibcal&0x3F))>>2, 0x0F);
    MOD_W2BYTEMSK(REG_MOD_BK00_2F_L,((U16)(u8cur_ibcal&0x3F))<<4, 0x3F0);    // ch16
    MOD_W2BYTEMSK(REG_MOD_BK00_2F_L,((U16)(u8cur_ibcal&0x3F))<<10, 0xFC00);    // ch17

    MOD_W2BYTEMSK(REG_MOD_BK00_30_L,((U16)(u8cur_ibcal&0x3F)), 0x3F);       // ch18
    MOD_W2BYTEMSK(REG_MOD_BK00_30_L,((U16)(u8cur_ibcal&0x3F))<<6, 0xFC0);   // ch19
    MOD_W2BYTEMSK(REG_MOD_BK00_30_L,((U16)(u8cur_ibcal&0x3F))<<12, 0xF000); // ch20
    MOD_W2BYTEMSK(REG_MOD_BK00_31_L,((U16)(u8cur_ibcal&0x3F))>>4, 0x03);

#if MOD_CAL_TIMER
    printf("[%s] takes %ld ms\n", __FUNCTION__, (MsOS_GetSystemTime()-delay_start_time));
#endif
    return (U8)MOD_R2BYTEMSK(REG_MOD_BK00_2D_L, 0x003F);
#endif
#else
    return 0;
#endif
...
static void MHal_PNL_PowerDownLPLL(U8 bEnable)
{
    if(bEnable)
    {
...
PNL_Result MHal_PNL_En(U8 bPanelOn, U8 bCalEn)
{
    U8 u8Cab;
    U8 u8BackUSBPwrStatus;
    if(bPanelOn)
...
void MHal_PNL_SetOutputPattern(U8 bEnable, U16 u16Red , U16 u16Green, U16 u16Blue)
{
    if (bEnable)
    {
...
void MHal_PNL_Switch_LPLL_SubBank(U16 u16Bank)
{
    UNUSED(u16Bank);
}
...
//  Variable
//-------------------------------------------------------------------------------------------------
//U32 gu32PnlRiuBaseAddr = 0;
//U32 gu32PMRiuBaseAddr = 0;
