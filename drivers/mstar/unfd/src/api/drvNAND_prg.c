...
void drvNAND_GetMtdParts(char *buf)
{
	...
	for(u8_i = 0; u8_i < pPartInfo->u16_PartCnt; u8_i ++)
	{
		...
		{
			case UNFD_PART_MIU:
				sprintf(PartName,"MIU%d",u8_PartNo );				
				break;
		...
		if (u8_i)
			sprintf(tmp, "0x%X(%s)", PartSize, PartName); //,pPartInfo->records[u8_i].u16_PartType);
		else 
			sprintf(tmp, "0x%X@0x%X(%s)", PartSize, 10 * pNandDrv->u16_BlkPageCnt * pNandDrv->u16_PageByteCnt, PartName); //,pPartInfo->records[u8_i].u16_PartType);
